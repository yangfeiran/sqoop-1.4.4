...
public class ImportTool extends com.cloudera.sqoop.tool.BaseSqoopTool {
	...
	private Object getMaxColumnId(SqoopOptions options) throws SQLException {
        ...
        //yangfeiran fix http://192.168.1.96:8080/browse/SG-107 start
		if (!options.getIncrementalTestColumn().toUpperCase().equals(options.getIncrementalTestColumn())
				&& (manager instanceof OracleManager)) {
			sb.append(manager.escapeTableName("\"".concat(options.getIncrementalTestColumn().concat("\""))));
		} else {
			sb.append(manager.escapeTableName(options.getIncrementalTestColumn()));
		}
		//yangfeiran fix http://192.168.1.96:8080/browse/SG-107 end
        sb.append(") FROM ");
	...
	private boolean initIncrementalConstraints(SqoopOptions options, ImportJobContext context)
		...
		case AppendRows:
			try {
				...
				if (isDateTimeColumn(checkColumnType)) {
					...
					throw new ImportException("Character column " + "(" + options.getIncrementalTestColumn()
							+ ") can not be used " + "to determine which rows to incrementally import.");
				} else {
			...
			break;
		case DateLastModified:
			checkColumnType = manager.getColumnTypes(options.getTableName(), options.getSqlQuery())
					.get(options.getIncrementalTestColumn());

			// cast to date type

			// if (options.getIncrementalLastType().equals("date")) {
			// checkColumnType = Types.DATE;
			// }
			...
			nextIncrementalValue = manager.datetimeToQueryString(nextVal.toString(), checkColumnType);
			break;
		default:
		...
		if (isDateTimeColumn(checkColumnType) && null != prevEndpoint && !prevEndpoint.startsWith("\'")
			...
			// around, here.
			prevEndpoint = manager.datetimeToQueryString(prevEndpoint, checkColumnType);
		}
	...
	protected boolean importTable(SqoopOptions options, String tableName, HiveImport hiveImport)
		...
		ImportJobContext context = new ImportJobContext(tableName, jarFile, options, getOutputPath(options, tableName));
	...
	public int run(SqoopOptions options) {
		...
		try {
			...
			LOG.error("Encountered IOException running import job: " + StringUtils.stringifyException(ioe));
			if (System.getProperty(Sqoop.SQOOP_RETHROW_PROPERTY) != null) {
	...
	protected RelatedOptions getImportOptions() {
		...
		if (!allTables) {
			...
			importOpts
					.addOption(OptionBuilder.withArgName("statement").hasArg()
							.withDescription(
		...
		importOpts
				.addOption(OptionBuilder.withArgName("n").hasArg()
						.withDescription(
								"Set number 'n' of rows to fetch from the " + "database when more rows are needed")
	...
	private void applyIncrementalOptions(CommandLine in, SqoopOptions out) throws InvalidOptionsException {
		...
		if (in.hasOption(INCREMENT_LAST_TYPE_ARG)) {
			out.setIncrementalLastType(in.getOptionValue(INCREMENT_LAST_TYPE_ARG));
		}
